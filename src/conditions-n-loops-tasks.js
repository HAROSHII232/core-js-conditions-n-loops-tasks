...
function isContainNumber(num, digit) {
  ...
  for (let i = 0; i <= numToString.length; i += 1) {
    if (numToString[i] === digitToString) return true;
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < resultMatrix.length; i += 1) {
    for (let j = 0; j < resultMatrix[i].length; j += 1) {
      resultMatrix[j][resultMatrix.length - i - 1] = startMatrix[i][j];
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  while (counter > 0) {
    ...
    for (let i = 0; i < newString.length; i += 1) {
      if (i % 2 === 0) {
        evenChars += newString[i];
      }
      if (i % 2 !== 0) {
        oddChars += newString[i];
      }
    }

    newString = evenChars + oddChars;
    counter -= 1;
